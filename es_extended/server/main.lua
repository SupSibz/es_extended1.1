...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	TriggerEvent('8Kx:giveitemalert',sourceXPlayer.name,targetXPlayer.name,ESX.Items[itemName].label,itemCount)
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			elseif Config.MaxCarryWeight < (ESX.CarryWeight(targetXPlayer) + (itemCount * ESX.Items[itemName].weight)) then
				TriggerClientEvent("pNotify:SendNotification", _source, {
					text = 'คุณ <strong class="red-text">' .. targetXPlayer.name ..'</strong> ไม่สามารถรับไอเทมจากคุณได้เนื่องจากกระเป๋าไม่พอเก็บ',
					type = "error",
					timeout = 3000,
					layout = "bottomCenter",
					queue = "global"
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.player.get('money') >= itemCount then
			...
			TriggerEvent('8Kx:givemoneyalert',sourceXPlayer.name,targetXPlayer.name,itemCount)
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerEvent('8Kx:giveblackmoenyalert',sourceXPlayer.name,targetXPlayer.name,Config.AccountLabels[itemName],itemCount)
...
RegisterServerEvent('Status:Give')
AddEventHandler('Status:Give', function(can)
		cangive = can	
end)
...
RegisterServerEvent('esx:Give')
AddEventHandler('esx:Give', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	MySQL.Async.execute(
		"DELETE FROM `owned_vehicles` WHERE `owner` = @iden", 
		{
	...
	MySQL.Async.execute(
		"DELETE FROM `users` WHERE `identifier` = @iden", 
		{
			["@iden"] = xPlayer.identifier
		}	
	)	
	...
	MySQL.Async.execute(
		"DELETE FROM `user_inventory` WHERE `identifier` = @iden", 
		{
			["@iden"] = xPlayer.identifier
		}	
	)	
