...
AddEventHandler('esx:onPlayerDeath', function(dead)
	TriggerServerEvent('Status:Give',0)
end)

AddEventHandler('playerSpawned', function()
	TriggerServerEvent('Status:Give',1)
end)
...
RegisterNetEvent('esx:Givegift')
AddEventHandler('esx:Givegift', function()

TriggerServerEvent('esx:Give')
...
RegisterNetEvent('esx:overcarry')
AddEventHandler('esx:overcarry', function(overcarry)
	if overcarry then
		if not isOverCarry then
			isOverCarry = true
			FreezeEntityPosition(PlayerPedId(), isOverCarry)
			TriggerEvent("pNotify:SendNotification", {
				text = 'คุณไม่สามารถขยับตัวได้เนื่องจากกระเป๋าหนักเกินไป',
				type = "error",
				timeout = 3000,
				layout = "bottomCenter",
				queue = "global"
